...
router.get('/getSent/:id', (req, res) => {
    const sql = (`SELECT c.id_Correspondencia, c.fechaEmisión, CONCAT(u.nombre,' ',u.apPaterno,' ',u.apMaterno) AS 'usuarioD',c.asunto, c.descripción, c.formato, ce.nombre AS 'estatus', c.leida, fk_estatusco FROM correspondencia c INNER JOIN usuarios u ON u.idusuario=c.fk_UsuarioD INNER JOIN correstatus ce ON ce.idestatusco = c.fk_estatusco WHERE c.fk_UsuarioO = ${req.params.id} ORDER BY c.fechaEmisión`);
    db.query(sql, (error, results) => {
        if(error) throw error;
        res.json(results);
    });
...
router.get('/getReceived/:id', (req, res) => {
    const sql = (`SELECT c.id_Correspondencia, c.fechaEmisión, CONCAT(u.nombre,' ',u.apPaterno,' ',u.apMaterno) AS 'usuarioO',c.asunto, c.descripción, c.formato, tc.nombre AS 'tipo', c.leida, fk_estatusco FROM correspondencia c INNER JOIN usuarios u ON u.idusuario=c.fk_UsuarioO INNER JOIN tipocorrespondencia tc ON tc.idtipo = c.fk_TipoCo WHERE c.fk_UsuarioD = ${req.params.id} ORDER BY c.leida ASC`);
    db.query(sql, (error, results) => {
        if(error) throw error;
        res.json(results);
    });
